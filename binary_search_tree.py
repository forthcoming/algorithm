...
"""
已知二叉树的前序遍历和中序遍历,如何得到它的后序遍历? 要利用以下几个特性:
1. 对于前序遍历,第一个肯定是根节点; 对于后序遍历,最后一个肯定是根节点
2. 利用前序或后序遍历,确定根节点,在中序遍历中,根节点两边就可以分出左子树和右子树
4. 对左子树和右子树分别做前面2点的分析和拆分,相当于递归,我们就可以重建出完整的二叉树
"""
    ...
    def post_order_stack(self):
        ...
        while stack:
            ...
            if root and root != r_child:
                ...
                r_child = stack.pop()
                print(r_child)
                # if not(r_child.left or r_child.right):  # root => leaf路径问题, 此处是后序遍历
                #     for i in stack:
                #         print(i,end=' ')
                #     print(r_child,end='\n')

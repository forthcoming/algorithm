...
def _encode_base58(v):
    ...
    v = v.lstrip(b'\0')  # Skip & count leading zeroes.
    new_len = len(v)
    # log(256) / log(58), rounded up.Allocate enough space in big-endian base58 representation.
    size = (new_len * 138 // 100 + 1)
    buffer = [0] * size
    length = 0
    for carry in v:
        right = size - 1
        ...
        while carry or _length < length:
            ...
            _length += 1
        length = _length
    string = bytearray(b'1' * (origen_len - new_len))
